...
def attack(
    ...
    print(model_name)
    save_perturb_iterations = sorted(set(list(range(900, iters, 200)) + [iters - 1])) if iters > 0 else []
    cropa_end = 300
    ...
    for id,item in enumerate(tpoch):            
        ...
        def get_value_vectors(pixel_values):
            ...
            if hasattr(eval_model.model, "vision_model"):
                with torch.cuda.amp.autocast(enabled=False):
                    vision_outputs = eval_model.model.vision_model(
                        pixel_values=pixel_values,
                        output_hidden_states=True,
                        return_dict=True,
                    )
                hidden_states = list(vision_outputs.hidden_states or [])
                if not hidden_states:
                    raise ValueError("vision_model did not return hidden states")
                # Skip the embedding output and keep the deepest hidden states for the D-UAP loss.
                start_idx = max(1, len(hidden_states) - 15)
                selected_hidden_states = hidden_states[start_idx:]
                return torch.cat(selected_hidden_states, dim=1)

            if hasattr(eval_model.model, "vision_encoder"):
            ...
            raise AttributeError("Unsupported model: no vision_model or vision_encoder found")
        ...
        image_dir = os.path.join(repo_root, "sdxl_gen_images")
        candidate_paths = [
            os.path.join(image_dir, f"{target_stem}.png"),
            os.path.join(image_dir, f"{target_stem}.jpg"),
            os.path.join(image_dir, f"{target_stem}.jpeg"),
        ]
        sdxl_result_path = next((path for path in candidate_paths if os.path.exists(path)), None)
        if sdxl_result_path is None:
            raise FileNotFoundError(
                f"Target image not found for '{target_stem}' in {image_dir}"
            )
...
if __name__=="__main__":
    ...
    parser.add_argument("--target", type=str, default="bomb",
