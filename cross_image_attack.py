...
def attack(
    ...
    # BUG FIX #1: Use same save_perturb pattern as main.py
    ...
    for ep in tqdm(range(iters), desc = "Epoch"):
        for id, item in enumerate(test_dataset):
            ...
            # BUG FIX #2: Track best_attack and best_loss per image
            best_attack = None
            ...
            # BUG FIX #3: Track loss and best_attack
        ...
        if ep not in save_perturb_iterations:
            continue
        # BUG FIX #4: Use best_attack (renamed from 'attack' to avoid shadowing)
        for id, item in enumerate(test_dataset):
            img_id = str(ques_id_to_img_id[item["question_id"]])
            attack_tensor = torch.tensor(noise_dict[ep],requires_grad=True,device=device)
            ...
            for i in range(len(prompt_list)):
                ...
                for batch_ques in more_itertools.chunked(instruction_list,args.eval_batch_size):
                    ...
                    # BUG FIX #5: Use 'attack_tensor' instead of 'attack'
                    outputs = eval_model.get_outputs_attack(
                                            attack = attack_tensor,batch_images=test_item_images*len(batch_ques),
    ...
    # BUG FIX #6: Add loss_json output
    mean_loss = np.mean([loss_json[i] for i in loss_json.keys()],axis=0) if loss_json else []
    if len(mean_loss) > 0:
