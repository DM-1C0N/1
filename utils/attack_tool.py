...
def add_extra_args(args, model_name, config_path = CONFIG_PATH):
    ...
    if model_name == "open_flamingo":
        ...
        args.eoc="</s>"
    elif model_name == "instructblip" or "idefics":
        args.prompt_num_to_alpha2 = {1:0.01,5:0.025,10:0.01,50:0.015,100:0.015}
        args.eoc = " </s>"       
...
def get_img_id_train_prompt_map(num):
    ...
    prompt_path = f'{query_path}/id_to_question.json'
    if not os.path.exists(prompt_path):
        os.makedirs(query_path, exist_ok=True)
        with open("data/filtered_v2_mscoco_val2014_annotations.json", "r") as f:
            annotations = json.load(f)["annotations"]
        image_ids = sorted({str(item["image_id"]) for item in annotations})
        id_to_prompts = {
            image_id: random.sample(train_question(), num) for image_id in image_ids
        }
        with open(prompt_path, "w") as f:
            json.dump(id_to_prompts, f)
...
def load_blip_model(device,module):
    ...
    local_blip_path = os.path.join(repo_root, "blip2-opt-2.7b")
    model_path = local_blip_path if os.path.isdir(local_blip_path) else 'Salesforce/blip2-opt-2.7b'
    model_args = {
        'lm_path': model_path,
...
def load_llava_model(device,module):
    model_args = {
        'lm_path': 'llava-hf/llava-1.5-7b-hf',         
...
def load_idefics_model(device,module):
    model_args = {
...
def load_model(device,module,model_name):
    ...
    if model_name=="blip2":
        ...
        return load_idefics_model(device, module)
    else:
